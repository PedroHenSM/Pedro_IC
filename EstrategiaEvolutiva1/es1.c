...
#define TIPO_ES 1// Se 0: es+ Se 1:es, Se 2: es, modificado
...
typedef struct {
    ...
    float sigma; // Strategy Parameters ( Parametros de Controle )
    ...
    float violacao; // Soma das violacoes
}Individuo;
...
void inicializaPopulacao(Individuo populacao[],int tamanhoX,int tamanhoPop){
    int i,j;
    ...
    for(i=0;i<tamanhoPop;i++){
        for(j=0;j<tamanhoX;j++){
            ...
            float val = eq * (rand() % 11); // Valores decimais entre [0,10] C01
...
void inicalizaEstrategiaEvolutiva(Individuo populacao[], Individuo filhos[]){
    int i;
    for(i=0;i<TAM_POPULACAO;i++){// Para Estratégia Evolutiva (ES)
        populacao[i].sigma = DESVIO_PADRAO;
        //filhos[i].sigma = DESVIO_PADRAO;
    }
...
void autoAdaptacaoSigma(Individuo populacao[],Individuo filhos[],int tamanhoX){
    ...
    /*for(i=0;i<TAM_POPULACAO;i++){
        ...
        //printf("EPSILON: %f \n",epsilon);
        filhos[i].sigma = populacao[i].sigma * exp(epsilon);
        for(j=0;j<tamanhoX;j++){
            filhos[i].x[j] = populacao[i].x[j] + filhos[i].sigma * randNormal(0,1);
        }
    }*/
    for(i=0;i<TAM_POPULACAO;i++){
        for(j=0;j<NUM_FILHOS_GERADOS;j++){ // Cada pai gera 5 filhos
            tau = 1/sqrt(tamanhoX);
            epsilon = tau*randNormal(0,1);
            filhos[l].sigma = populacao[i].sigma * exp(epsilon);
            for(k=0;k<tamanhoX;k++){
                filhos[l].x[k] = populacao[i].x[k] + filhos[l].sigma * randNormal(0,1);
            }
            l++;
        }
    }
...
void ordenaMelhores(Individuo populacao[],Individuo filhos[]){
    ...
    if(TIPO_PROBLEMA == 0){ // Rosenbrock
        ...
        qsort(populacao,TAM_POPULACAO,sizeof(Individuo),comparaFuncaoObjetivo2); // TODO: Verificar funcao de comparacao(fabs)
...
void copiaIndividuos(Individuo filhos[],Individuo populacao[],int inicioDaCopia,int tamanhoX){ // Copia de filhos para populacao
    // Inicio da copia: A partir de onde ser� copiado
    int i,j,k;
    for(i=0,k=inicioDaCopia;k<TAM_POPULACAO;i++,k++){
        populacao[k].sigma=filhos[i].sigma;
...
void estrategiaEvolutiva(){
    ...
    srand(SEED);
    inicializaPopulacao(populacao,TAM_X,TAM_POPULACAO);
    avaliaFuncao(populacao,&contaObj,TAM_X,TAM_POPULACAO);
    inicalizaEstrategiaEvolutiva(populacao,filhos);
...
void estrategiaEvolutivaGeraDados(int seed,int tamanhoX,int tipoES){
    ...
    Individuo filhos[TAM_POPULACAO_FILHOS];
    srand(seed);
    inicializaPopulacao(populacao,tamanhoX,TAM_POPULACAO);
